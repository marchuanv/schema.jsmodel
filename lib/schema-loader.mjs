...
import { getPathSegments } from './path-segments.mjs';
    ...
    */
    constructor(jsonSchemaFilePath) {
        ...
        if (!existsSync(_jsonSchemaFilePath)) {
            ...
            let jsonSchemaFilePathMatches = [];
            walkDir(runningDirPath, (filePath) => {
                const pathSegments = getPathSegments(filePath);
                const sourcePathSegmentsLength = sourcePathSegments.length;
                for (let index = sourcePathSegmentsLength; index === 0; index--) {
                    if (sourcePathSegments[index] === pathSegments[index]) {
                        jsonSchemaFilePathMatches.push(filePath);
                    }
                }
            });
            if (jsonSchemaFilePathMatches.length !== 0) {
                throw new Error(`more than one file match for: ${_jsonSchemaFilePath}`);
            }
            _jsonSchemaFilePath = jsonSchemaFilePathMatches[0];
        }
        privateBag.set(this, { jsonSchemaFilePath: _jsonSchemaFilePath, isLoaded: false, schema: null, Id: null });
    }
    /**
     * @returns { Boolean }
    */
