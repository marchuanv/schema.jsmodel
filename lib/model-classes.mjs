...
export class ModelClasses {
    ...
    */
    static create(schemaLoader) {
        ...
        const modelsDirPath = path.join(schemaLoader.schemaDirPath, `${schemaLoader.schemaName}-models`);
...
function createModelClass(modelsDirPath, schema) {
    ...
    for (const propertyName of Object.keys(properties)) {
        const { type, $ref } = properties[propertyName];
        let typeMap = null;
        if ($ref) {
            ...
            if (typeMap.prototype === Object.prototype) {
                ...
                TypeMap.register(modelClassName, null, 'object');
                typeMap = new TypeMap(modelClassName);
            }
        } else {
            throw new Error(`unable to resolve type for property: ${propertyName}`);
        }
